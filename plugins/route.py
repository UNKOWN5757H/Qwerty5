...
async def watch_page_handler(request: web.Request):  # <-- RENAMED THIS FUNCTION
    """
    Serves the HTML watch page.
    """
    try:
...
async def stream_handler(request: web.Request):  # <-- This is the main file streaming function
    """
    Serves the actual file stream.
    """
    try:
        path = request.match_info["path"]
        match = re.search(r"^([a-zA-Z0-9_-]{6})(\d+)$", path)
...
async def media_streamer(request: web.Request, id: int, secure_hash: str):
    ...
    if mime_type:
        ...
        if file_name:
            mime_type = mimetypes.guess_type(file_id.file_name)[0] # Ensure it returns a string
        else:
            mime_type = "application/octet-stream"
            file_name = f"{secrets.token_hex(2)}.unknown"
            
    # Fallback if mime_type is still None
    if not mime_type:
        mime_type = "application/octet-stream"
